...
//		console.log(projectName);
		rdfmgr.executeSparql({//RDFmgrに検索式を渡して検索実行を要求 クラス名の取得
...
//						$("#aaa").append("<div id="+i+" onclick='test_ukai(this)' style='border-style: ridge; border-width: 2px;padding: 10px 5px 10px 5px;word-wrap: break-word;'><div>"+json.getValue(i)+"</div></div><br>");//divタグ内文章折り返し、余白、枠線
						var title = json.getValue(i).replace('lodcu:','');
						console.log(title);
					}
				}
			},
		...
		rdfmgr.executeSparql({//RDFmgrに検索式を渡して検索実行を要求
			sparql: "select ?s ?p ?o where{?s a <http://lodcu.cs.chubu.ac.jp/lodcu#DisasterInfo>.?s ?p ?o.}",
//			sparql: "select ?s ?p ?o where{?s a <http://lodcu.cs.chubu.ac.jp/lodcu#"+title+">.?s ?p ?o.}",
			inference:false,
			remakeJson: false,
			success: test_ukai,
			error:function(eType, eMsg, eInfo){
...
/*		rdfmgr.executeSparql({//RDFmgrに検索式を渡して検索実行を要求
			sparql: "select ?s ?p ?o where{?s a <http://lodcu.cs.chubu.ac.jp/lodcu#Shelter>.?s ?p ?o.}",
			inference:false,
			remakeJson: false,
			success: test2_ukai,
			error:function(eType, eMsg, eInfo){
...
*/
	function test_ukai(json){
		console.log("test鵜飼スタート");
		var s=null;
		var p;
		var o;
		var s2=null;
		...
		while(json.next()){
//			for(var i=0;i<json.getLength();i++){//iの変動0.1.2}
			s = json.getValue(0);
			p = new String(json.getValue(1));
			o = json.getValue(2);
			if(s2!=null&&s2!=s){
				console.log("----------------------------------------------");
				console.log(s2);
...
/*					console.log(lat);
					...
					console.log(insname);
					console.log(time);*/
					placemarker_ukai(lat,lon,insname,time);

/*					$("#aaa").append("<div id="+i+" onclick='test_ukai2(this)' style='border-style: ridge; border-width: 2px;padding: 10px 5px 10px 5px;word-wrap: break-word;'><div>"+insname+"</div><div>at "+time+"</div></div><br>");//divタグ内文章折り返し、余白、枠線
					$('div#'+i+'').attr({
						'name': insname,
						'time': time,
						'lat': lat,
			...
			if(p.search("経度")!="-1"||p.search("lng")!="-1"||p.search("long")!="-1"||p.search("lon")!="-1"||p.search("longitude"||p.search("geo:long")!="-1")!="-1"){
				var lon = o;
			};
...
//				$("#aaa").append("<div id="+key+" onclick='test2(this)' style='border-style: ridge; border-width: 2px;padding: 10px 5px 10px 5px;word-wrap: break-word;'><div>"+label+"</div><div>in "+LOD_label+"</div></div><br>");//divタグ内文章折り返し、余白、枠線
			...
			i++;
//			console.log(i);
			if(i==json.getValueListLength()){//最後用
				console.log("----------------------------------------------");
				console.log(s2);
				if(lat!=null&&lon!=null){
/*					console.log(lat);
...
/*					$('div#'+i+'').attr({
	...
	function test_ukai2(info){
		...
		currentId=info.id;
		var name = $('#'+info.id+'').attr('name');
		var time = $('#'+info.id+'').attr('time');
		var lat = $('#'+info.id+'').attr('lat');
		var lon = $('#'+info.id+'').attr('lon');
		placemarker_ukai(lat,lon,name,time);
	}


	function placemarker_ukai(lat,lon,name,time){
//		console.log(lat);
//		console.log(lon);
//		var str=table;
//		console.log(name);
...
//			animation:google.maps.Animation.DROP
		...
		setMarkerListener_ukai(marker,time);
//		createMarkerButton_linker(marker);
		createMarkerButton_ukai(marker);
		gTrackMarkerList.push(marker);
	}

	function setMarkerListener_ukai(marker,time){
...
/*
	...
	//右カラムにサイドバーを作成する
	function createMarkerButton_ukai(marker) {
