...
var gMarkerClusterer;// = new MarkerClusterer();//----------------------------大量のデータを取り扱うときに
var gMarkerCenter;
var infoWindow;
var geolatlong=false;
...
if(projectName == ""){
	projectName = "evacuationResilience"
//	projectName = "testKU";                               *** Old Project ***
}
if(setting_lat == ""){
	setting_lat = "lat";
}
...
//			console.log(json.getValueListLength());
			if(json.getValueListLength()==0){
				var namejson2 = new String('[');
				namejson2 += '{"classname":"All Clear"}]';
				var linkdata_all = JSON.parse(namejson2);
				SparqlSend_linkdata1(linkdata_all)
			}else{
			while(json.next()){
...
//	console.log(rowsnum);
	...
	for(var i=0;i<num;i++){//checkされてる分
		...
		if(check[i].classname==rows[j].classname){
			...
			$('#'+j).html(str);
			rdfmgr.executeSparql({//RDFmgrに検索式を渡して検索実行を要求
				//?x ?status ?disaster ?name ?comment ?time ?userid ?lat ?lng
				sparql: "SELECT ?x ?status ?disaster ?name ?comment ?time ?userid ?lat ?lng WHERE{?x rdf:type lodcu:" + title +"." +
				"?x lodcu:name ?name." +
				"?x lodcu:lat ?lat." +
				"?x lodcu:lng ?lng." +
				"OPTIONAL{?x lodcu:time ?time." +
				"?x lodcu:status ?status." +
				"?x lodcu:comment ?comment." +
				"?x lodcu:userid ?userid." +
				"?x lodcu:disaster ?disaster.}}order by ?time",
				inference:false,
				remakeJson: false,
				success: json_analyze,
				error:function(eType, eMsg, eInfo){
...
//	console.log(datacheck);
	...
	var l = json.getKeyListLength();

	while(json.next()){
//		console.log(json.getValue("pro"));

		if(typeof obj[json.getValue("x")] === "undefined"){
			obj[json.getValue("x")] = {};

			for(var i = 1;i < l;i++){
				if(json.getValue(i) != undefined){
					obj[json.getValue("x")][json.getKey(i).replace('lodcu:','')] = json.getValue(i);
				}
			}
		}
		nomap = false;
...
//					console.log("クラッシュテストmarker3");console.log(i);
				placeMarker3(obj[i],i);
				}else{
//					console.log("クラッシュテストmarker2");
					placeMarker2(obj[i],i);
				}
			}else{
				if(geolatlong==true){
//					console.log("クラッシュテストmarker3");console.log(i);
					placeMarker4(obj[i],i);
				}else{
//					console.log("クラッシュテストmarker2");
					placeMarker(obj[i],i);
				}
...
//	$('#qsel3').append($("<option></option>").val(obj[setting_name]).html(obj[setting_name]));
	...
	console.log("マッピングテスト");

	var image=new google.maps.MarkerImage(
			  pingnum//画像のURL
/*			  new google.maps.Size(32.0,32.0),  //画像のサイズ
		...
		var shadow=new google.maps.MarkerImage(
				...
				new google.maps.Point(0,0.0)     //影の画像のアンカーの位置
			);
//	console.log(table);
...
//	console.log("クラッシュテストplacemarker2");
//	console.log("placeMarker()スタート");
/*	console.log(obj);
	console.log(obj[setting_lat]);
	console.log(obj[setting_long]);*/
//最初からマップを表示したくないときはここでmapオブジェクトをつくるといい
//	console.log(obj);
/*
	for (keys in obj){
...
function placeMarker3(obj,num){//geolat long用
console.log("クラッシュテスト");
//	console.log("placeMarker()スタート");
...
//		gMarkerClusterer = new MarkerClusterer(mapObj);//----------------------------大量のデータを取り扱うときに便利だよ用
	...
	var name = obj[setting_name];
	//console.log(obj["geo:lat"]);
	//console.log(obj["geo:long"]);
	var location = new google.maps.LatLng( obj["geo:lat"], obj["geo:long"]);
	markerBounds.extend(location);//---------------------------------表示領域のための関数fitBounds()用
	var table = maketable(false,obj,num);
	var infowindow = new google.maps.InfoWindow({
		disableAutoPan:false,
...
function placeMarker4(obj,num){//markercluster使う　geolatlong
//	console.log("placeMarker()スタート");
...
function settingedit(){
	...
	setting_proID = $("#setting_id").val();
	setting_lat = $("#setting_lat").val();
	setting_long = $("#setting_long").val();
	setting_name = $("#setting_name").val();
	/*
	rdfmgr.setDefaultProject($("#setting_id").val());
	$("#qsel5").empty();
	$("#qsel5").append($("<option></option>").html("選択してください"));
	queryRead();
	*/
	...
	var params = "?project="+setting_proID;
	params += "&name="+setting_name;
	params += "&lat="+setting_lat;
	params += "&long="+setting_long;
